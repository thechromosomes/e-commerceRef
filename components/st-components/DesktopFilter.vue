  ...
  <client-only>
    ...
    >
      <div class="st-dropdown-area">
        <div
          class="st-dynamic-filters"
          v-if="filter.type === 'textFacet'"
          :class="{ 'st-colors': filter.title === 'Color' }"
        >
          <div class="st-checkbox" v-for="i in filter.selected" :key="i">
            <label class="active">
              <input
                type="checkbox"
                :value="i"
                @change="$emit('applyFilter')"
                v-model="filter.selected"
              />
              <span
                class="st-item-checkbox"
                v-if="filter.title !== 'Color'"
              ></span>
              <span class="st-item-checkbox" v-else></span>
              <span class="filter-name">{{ i }}</span>
              <span class="st-count-filter"
                >({{ getSelectedCount(i, filter) }})</span
              >
            </label>
          </div>
          <div
            class="st-checkbox"
            v-for="i in getNotSelectedFilters(filter)"
            :key="i.name"
          >
            <label>
              <input
                ...
                @change="
                  $emit('applyFilter'),
                    filterAnalytics('left', filter.title, i.name)
                "
                v-model="filter.selected"
              />
              <span
          ...
          >
            <label>
              <input
                ...
                @change="
                  $emit('applyFilter'),
                    filterAnalytics('left', filter.title, [i.min, i.max])
                "
                v-model="filter.selected"
              />
              <span class="st-item-checkbox"></span>
              <span class="filter-name"
                >{{ currSymbol }}{{ i.min }}
                <span v-if="i.min !== '60000'"
                  >- {{ currSymbol }}{{ i.max }}</span
                >
                <span v-else> And Above</span>
              </span>
              <span class="st-count-filter">({{ i.count }})</span>
            ...
            <div class="outer-checkbox">
              <label class="st-flex">
                <input
                  ...
                  @change="
                    $emit('applyFilter'),
                      filterAnalytics('left', filter.title, i)
                  "
                />
            ...
            <div class="outer-checkbox">
              <label class="st-flex">
                <input
                  ...
                  @change="
                    $emit('applyFilter'),
                      filterAnalytics('left', filter.title, [i[0], i[1]])
                  "
                />
                <span class="st-checkbox"></span>
...
export default {
  ...
  methods: {
    ...
    getSelectedValues: function (filter) {
      ...
      if (filter.selected.length > 0) {
        if (filter.type === "textFacet")
          arr = filter.selected.toString().replaceAll(",", ", ");
        else if (filter.type === "numericFacet") {
          filter.selected.forEach((x) => {
            if (x[1] !== "10000000")
              arr += `${this.currSymbol}${x[0]} - ${this.currSymbol}${x[1]}, `;
            else if (x[1] === "10000000")
              arr += `${this.currSymbol}${x[0]} And Above, `;
          });
    ...
    getSelectedCount: function (selected, filter) {
      ...
      if (cat !== undefined) return cat.count;
      else return 0;
    },
